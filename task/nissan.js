...
2024-06-18 用第三方API提交，解决QX提交问题(此方法纯属无奈，会泄露Token)
...
async function main() {
  ...
  if ($.push_id) {
    ...
    if ($.isQuanX()) {
      var {result, msg, rows} = await coolaf('put');
      message += `关注博主：${msg}\n`;
      var {result, msg, rows}= await coolaf('delete');
      message += `取消关注：${msg}\n`;
    }else {
      // put/delete
...
async function coolaf(method) {
  url = 'http://coolaf.com/tool/ajaxpost';
  appversion = $.appversion || '3.1.5';
  timestamp = Math.floor(Date.now() / 1000);
  noncestr = getNonce();
  sign = CryptoJS.SHA512(`nissanapp${timestamp}${$.token}${noncestr}1${$.uuid}`).toString();
  nissan_url = `https://oneapph5.dongfeng-nissan.com.cn/mb-gw/dndc-gateway/community/api/v2/user/followings/${$.user_id}`;
  Boundary = getRandomString(16);
...
utf8
------WebKitFormBoundary${Boundary}
Content-Disposition: form-data; name="cy"
...
`;
  ...
  const {code, msg, data} = await httpRequest(rest);
  return $.toObj(data?.response)
}
